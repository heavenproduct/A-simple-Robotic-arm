...
lower_red = np.array([154, 86, 0])
upper_red = np.array([255,255, 255])

lower_red = np.array([94, 151, 46])
upper_red = np.array([164,255, 255])

lower_green = np.array([30, 107, 47])
...
while True:
    ...
    if len(cnts) > 0:  # 如果检测出了轮廓
        #for c in cnts:
            ...
            print(center)
            if radius >5 :
                cv2.rectangle(img, (int(x)-10, int(y)-10), (int(x + 10), int(y + 10)), (255, 0, 0), 1)
    ...
    cv2.imshow("res", res)
    image = Image.fromarray(cv2.cvtColor(res, cv2.COLOR_BGR2RGB))
    #image.save('res.jpg')
